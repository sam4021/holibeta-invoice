                        ...
                        <form @submit.prevent="submit" id="saveFacilities">
                            <div class="mx-6 grid grid-cols-1 md:grid-cols-2 gap-3">
                                ...
                                <div>
                                    ...
                                    <input @input="form.visual_inspection_image=$event.target.files[0]" class="block w-full text-sm  border border-gray-300 rounded-lg cursor-pointer bg-gray-50  focus:outline-none my-2" id="visual_inspection_image" type="file">
...
const submit = () => {
    form.post(route('quality-control.store'),{
        preserveScroll:true,
        onSuccess:()=>{
            show.value=false
            form.reset()
