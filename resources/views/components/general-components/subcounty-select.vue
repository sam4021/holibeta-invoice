<template>
    <div class="relative" v-on-click-outside="closeDrop">
        ...
        justify-between w-full  items-center" :class="[disabled?'opacity-80':'']">
            <button @click="subcountyDropDown=!subcountyDropDown"  type="button" class="font-medium w-full h-full text-start">
                <span class="self-center" v-if="locationStore.subcounty">{{locationStore.subcounty.name}}</span>
                <span class="self-center" v-else >{{placeholder}}</span>
            </button>
            <svg v-if="locationStore.subcounty" @click="resetSelected"  class="h-4 fill-black" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512"><path d="M312.1 375c9.369 9.369 9.369 24.57 0 33.94s-24.57 9.369-33.94 0L160 289.9l-119 119c-9.369 9.369-24.57 9.369-33.94 0s-9.369-24.57 0-33.94L126.1 256L7.027 136.1c-9.369-9.369-9.369-24.57 0-33.94s24.57-9.369 33.94 0L160 222.1l119-119c9.369-9.369 24.57-9.369 33.94 0s9.369 24.57 0 33.94L193.9 256L312.1 375z"/></svg>
        ...
        >
            <div v-show="subcountyDropDown && !disabled" class="py-2 mt-1 absolute z-10 bg-white divide-y divide-gray-100 rounded-lg
            shadow w-full overflow-hidden ">
                ...
                <ul v-if="!locationStore.loading" class="text-sm text-gray-700 max-h-48 overflow-y-auto divide-y" aria-labelledby="dropdownHoverButton">
                    <li v-for="(subcounty,index) in filteredSubcounties" :key="index" class="grid cursor-pointer font-bold text-xs">
                        <div class="my-2 p-1">
                            <label class="cursor-pointer flex  gap-2 font-bold text-xs">
                                <input  @input="subcountyDropDown=false" :value="subcounty" type="radio" v-model="locationStore.subcounty" class="hidden">
...
const subcountyDropDown=ref(false)
const resetSelected=()=>{
    locationStore.subcounty=''
    subcountyDropDown.value=false
    search.value=''
}
const closeDrop=()=>{
    subcountyDropDown.value=false
...
let filteredSubcounties = computed(() =>
    search.value === ''
        ? locationStore.subCounties
        : locationStore.subCounties?.filter((subcounty :string) =>
