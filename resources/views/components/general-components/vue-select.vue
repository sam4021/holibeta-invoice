<template>
    <div class="relative" v-on-click-outside="closeDrop">
        ...
        justify-between w-full  items-center" :class="[disabled?'opacity-80':'']">
            ...
            <svg v-if="active" @click="resetSelected"  class="h-4 fill-black" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512"><path d="M312.1 375c9.369 9.369 9.369 24.57 0 33.94s-24.57 9.369-33.94 0L160 289.9l-119 119c-9.369 9.369-24.57 9.369-33.94 0s-9.369-24.57 0-33.94L126.1 256L7.027 136.1c-9.369-9.369-9.369-24.57 0-33.94s24.57-9.369 33.94 0L160 222.1l119-119c9.369-9.369 24.57-9.369 33.94 0s9.369 24.57 0 33.94L193.9 256L312.1 375z"/></svg>
        ...
        >
            ...
            shadow w-full overflow-hidden ">
                ...
                <ul class="text-sm text-gray-700 max-h-48 overflow-y-auto" aria-labelledby="dropdownHoverButton">
                    <li v-for="option in filteredOptions" :key="option.id" class="grid ">
                        <button type="button" @click="selectOption(option)" class="text-start font-medium w-full h-full cursor-pointer  hover:bg-sumo-700/20 p-2">
...
const intialSelect=computed(()=>{
    return  props.options?.find((item: { id: number | undefined; }) => item.id === props.selected)
})
...
function selectOption(option:any){
    ...
    emits('update:selected',option.id)
    search.value=''
...
let filteredOptions = computed(() =>
    search.value === ''
        ? props.options
        : props.options?.filter((option: { name: string; }) =>
            option.name
                .toLowerCase()
                .replace(/\s+/g, '')
