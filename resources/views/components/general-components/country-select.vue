    ...
    <div class="relative" v-on-click-outside="closeDrop" ref="dropbox">
        ...
        justify-between w-full  items-center" :class="[disabled?'opacity-80':'']">
            <button @click="dropdown=!dropdown"  type="button" class="font-medium w-full h-full text-start">
                <span class="self-center" v-if="locationStore.default_country">{{locationStore.default_country.name}}</span>
                <span class="self-center" v-else >{{placeholder}}</span>
            </button>
            <svg v-if="locationStore.default_country" @click="resetSelected"  class="h-4 fill-black cursor-pointer" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512"><path d="M312.1 375c9.369 9.369 9.369 24.57 0 33.94s-24.57 9.369-33.94 0L160 289.9l-119 119c-9.369 9.369-24.57 9.369-33.94 0s-9.369-24.57 0-33.94L126.1 256L7.027 136.1c-9.369-9.369-9.369-24.57 0-33.94s24.57-9.369 33.94 0L160 222.1l119-119c9.369-9.369 24.57-9.369 33.94 0s9.369 24.57 0 33.94L193.9 256L312.1 375z"/></svg>
        ...
        >
            ...
            shadow w-full overflow-hidden ">
                ...
                <ul v-if="!locationStore.loading" class="text-sm text-gray-700 max-h-48 overflow-y-auto" aria-labelledby="dropdownHoverButton">
                    <li v-for="country in locationStore.countries" :key="country.id" class="grid ">
                        <button type="button" @click="selectOption(country)" class="text-start font-medium w-full h-full cursor-pointer  hover:bg-sumo-700/20 p-2">
                            <span class="capitalize">{{country.name}}</span>
                        </button>
...
function selectOption(country:any){
    dropdown.value=false
    locationStore.default_country=country
    search.value=''
    locationStore.getCities(country.name)

}
const resetSelected=()=>{
   locationStore.default_country={}
    dropdown.value=false
    search.value=''
}
const closeDrop=()=>{
