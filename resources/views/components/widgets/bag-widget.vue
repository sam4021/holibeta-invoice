...
const options=computed(()=>({
    xaxis: {
        categories: bags.value.map((item: { day: any; })=>item.day)
    },
    chart: {
        id: 'Daily Bags'
    },
...
const series=computed(()=>([
    {
        name: 'Bags',
        data: bags.value.map((item: { data: any; })=>item.bags)
    }
]))

const company = computed(() => usePage().props.auth.company.company_id)

const bagsLoading=ref(false)

const getBag= async ()=>{
    bagsLoading.value=true
    await  axios.get(`/api/get/warehouse/bags/daily`).then((response)=>{
        bags.value=response.data
        console.log(response.data);
        
        bagsLoading.value=false
    ...
    <card>
        ...
        <template #header>
            <div class="flex justify-between items-center">
                <div>
                    <h6 class="text-md font-black text-sumo-300">Daily Bags for the last 5 days</h6>
                </div>
                <div>
                    <custom-dropdown>
                        <template #trigger>
        ...
        <template #footer>
            <div class="flex justify-end p-3">
                <Link :href="route('warehouse.bags')" class="btn-simple btn-small">
                    <span>See all</span>
                </Link>
            </div>
        </template>
