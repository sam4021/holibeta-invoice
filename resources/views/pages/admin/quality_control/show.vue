...
import UpdateQc from "@/views/components/quality-control/update.vue";
...
let image = '/images/quality-control/'+props.qc.data.visual_inspection_image
</script>
...
<admin>
    <div class="flex justify-between items-center">
        <div>
            <h1 class="text-2xl font-bold capitalize">Quality Control Data</h1>
        </div>
        <div class="flex gap-2">
           <div>
            <update-qc :qc="qc.data">
                                <template #trigger>
            ...
            <div>
                <prompt-alert
                    title="Are you sure you want to delete this Quality Control?"
                    description="All related data will be deleted"
                    @proceed="deleteQc(qc.data.id)"
                >
                    <template #trigger>
                ...
                <div class="my-3">
                    <table class="text-sm font-medium">
                        <tr>
                            ...
                            <td class="px-3">
                                <Link :href="route('weighbridge.show', qc.data.weighbridge.id)"
                                              class="flex items-center gap-1">
                                            {{  qc.data.weighbridge.code }}
                        ...
                        <tr>
                            ...
                            <td class="px-3">{{qc.data.created_by.name}}</td>
                        </tr>                        
                        <tr>
                            <th class="text-start">Date:</th>
                            <td class="px-3">{{ moment(qc.data.created_at).format("DD MMM, YYYY h:MM a") }}</td>
                        </tr>                      
                    </table>
                </div>
                ...
                <div class="my-3">
                    <table class="text-sm font-medium">
                        <tr>
                            ...
                            <td class="px-3">{{qc.data.aflatoxin_content}}</td>
                        </tr>
                        <tr>
                            <th class="text-start">Moisture Content:</th>
                            <td class="px-3">{{qc.data.moisture_content}}</td>
                        </tr>
                        <tr>
                            <th class="text-start">Visual Inspection:</th>
                            <td class="px-3">{{qc.data.visual_inspection}}</td>
                        </tr>
                        <tr>
                            <th class="text-start">Visual Inspection Comment:</th>
                            <td class="px-3">{{qc.data.visual_inspection_comment}}</td>
                        </tr>
                        <tr>
                            <th></th>
                            <td><img :src="image"></td>
